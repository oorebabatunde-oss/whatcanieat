    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="pt-6 pb-2 px-4 text-center">
        <button onClick={onBack} className="inline-block">
          <h1 className="text-xl font-display font-bold text-primary">What Can I Eat?</h1>
        </button>
      </header>
          ...
          {!preview ? (
            ...
            >
              ...
              <p className="text-muted-foreground text-sm text-center -mt-4">
                Take a photo or upload an image of your fridge, pantry, or ingredients
              </p>
            ...
            >
              ...
              {loading ? (
                <div className="flex flex-col items-center gap-3 py-4">
                  ...
                  <p className="text-muted-foreground text-sm">Identifying ingredients...</p>
                </div>
              ) : (
            ...
            >
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs -mt-2 mb-3">
                  AI-generated suggestions based on your ingredients
                </p>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-1">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <Badge className={`text-[10px] px-1.5 py-0 ${DIFFICULTY_COLOR[recipe.difficulty]}`}>
                            {recipe.difficulty}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
                        ...
                        <span className="flex items-center gap-1">
                          <BarChart3 className="w-3 h-3" /> {recipe.usesIngredients.length} ingredients
                        </span>
                      </div>

                      <div className="flex flex-wrap gap-1 mb-3">
                        {recipe.usesIngredients.map((ing) => (
                          <span key={ing} className="text-[11px] bg-muted px-2 py-0.5 rounded-full text-muted-foreground">
                            {ing}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => openRecipeSearch(recipe.name)}
                        >
                          <ExternalLink className="w-3.5 h-3.5" /> Recipe
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-xs gap-1.5 flex-1"
                          onClick={() => openYouTube(recipe.name)}
                        >
